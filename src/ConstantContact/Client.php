...
class Client
{
    ...
    public function addScope(string $scope): self
    {
        if (!\in_array($scope, $this->validScopes)) {
            throw new \PHPFUI\ConstantContact\Exception\InvalidParameter(
                "Scope {$scope} is not valid, must be one of (".\implode(',', $this->validScopes).')'
            );
        }
    ...
    public function getAuthorizationURL(): string
    {
        ...
        if ($this->PKCE) {
            list($code_verifier, $code_challenge) = $this->codeChallenge();
        ...
        $url = $this->authorizeURL.'?'.\str_replace(
                '%2B',
                '+',
                \http_build_query($params)
            );    // hack %2B to + for stupid CC API bug
     ...
     */
    private function codeChallenge( $code_verifier = null): array
    {
        $gen = static function () {
    ...
    private function session(string $key, string $value): string
    {
        ...
        if (PHP_SESSION_ACTIVE !== \session_status()) {
            throw new \PHPFUI\ConstantContact\Exception(
                'session not started. Call session_start() or use \PHPFUI\ConstantContact\Client->setSessionCallback'
            );
        }
